...
namespace Tools {
    ...
    unsigned long HookFunc(void *target, void *replace, void **backup);
...
bool SetProtRWX(void *addr) {
	unsigned long page_size = sysconf(_SC_PAGESIZE);
	unsigned long size = page_size * sizeof(uintptr_t);
	void *p = (void *) ((uintptr_t) addr - ((uintptr_t) addr % page_size) - page_size);
	return mprotect(p, (size_t) size, PROT_EXEC | PROT_READ | PROT_WRITE) == 0;
}

unsigned long Tools::HookFunc(void *address, void *replace, void **backup) {
	if (SetProtRWX(address)) {
		//return DobbyHook(address, replace, backup);
        return (unsigned long)((uint64_t)DobbyHook(address, replace, backup));
	}
    //return DobbyDestroy(address);
    return (unsigned long)((uint64_t)DobbyDestroy(address));
}
